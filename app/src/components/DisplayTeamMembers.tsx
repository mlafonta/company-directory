        ...
        <Container>
            <Grid container spacing={2} justifyContent="center" sx={{marginTop: 8}}>
                ...
                <Grid xs={8} item container justifyContent="center">
                    {!loading && member?.map((member:IUser, key) => (
                        <Grid item container key={key} xs={6} style={{marginBottom: 10}} justifyContent="center" flexDirection="column">
                            <Typography
                                className="list"
                                component="a"
                                href={`/user/${member.id}`}
                            ...
                            <Typography variant="h5" display="block">{member.position}</Typography>
                        </Grid>
                    ))}
            </Grid>
                <Grid xs={2} item>
